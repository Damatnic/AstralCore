    ...
    const checkConsents = useCallback(async () => {
        try {
            ...
            for (const docType of requiredDocs) {
                const consent = await ApiClient.legal.checkConsent(currentUserId, docType);
                const requiredVersion = LEGAL_DOC_VERSIONS[docType as keyof typeof LEGAL_DOC_VERSIONS];
                if (!consent || consent.document_version !== requiredVersion) {
                    setRequiredConsent(docType);
                    setAllConsentsGiven(false);
                    return;
                }
            }
