...
import { ChatSession, Dilemma, Helper } from '../types';
import { BackIcon, SendIcon, CertifiedIcon, HeartIcon, CrisisIcon } from '../components/icons';
...
export const ChatView: React.FC<{
    ...
    const { helpSessions, toggleFavorite, startVideoChat } = useSessionStore();
                ...
                navigator.geolocation.getCurrentPosition(
                    ...
                    (error) => {
                        ...
                        showConfirmationModal({
                            ...
                            message: 'Could not get your location. Send alert without location data?',
                            confirmText: 'Yes, Send Anyway',
                            confirmVariant: 'danger',
                            onConfirm: () => {
                                ApiClient.emergency.trigger(dilemma.id);
                                addToast("Emergency alert sent without location data.", 'info');
                            }
                        })
                    },
    ...
    return (
        <div className="chat-view">
                ...
                <div className="chat-header-info">
                    <h2>
                        {helperForChat && perspective !== 'helper' ? (
                            <a href="#" onClick={(e) => { e.preventDefault(); onViewHelperProfile(helperForChat.id); }}>
                                {getHeaderTitle()}
                            </a>
                        ) : getHeaderTitle()}
                    </h2>
            ...
            <div className="chat-messages">
                 {guidance?.isCrisis && perspective === 'seeker' && <CrisisAlert />}
            ...
            <div className="chat-composer" style={{position: 'relative'}}>
                ...
                />
                <AppButton onClick={handleSend} disabled={!newMessage.trim()} className="chat-send-btn">
                    <SendIcon />
                </AppButton>
            </div>
