...
describe('ShareView user flow', () => {
  ...
  test('user can chat with AI, draft a post, and submit it', async () => {
    ...
    expect(await screen.findByText(/That sounds really tough/)).toBeInTheDocument();
    ...
    expect(await screen.findByLabelText(/Your anonymous post/i)).toBeInTheDocument();
    
    const textArea = screen.getByLabelText(/Your anonymous post/i);
    expect(textArea).toHaveValue("This is the drafted post from the AI.");
    
    // --- 3. User edits and submits the form ---
    await userEvent.type(textArea, " And I added this extra text.");
    expect(textArea).toHaveValue("This is the drafted post from the AI. And I added this extra text.");
